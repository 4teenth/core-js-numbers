...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  // A(x1, y1), B(x2, y2)
  // Make a triangle
  // AC = x2 - x1, BC = y2 - y1, so use Pifagor theorema AB = VAC^2 + BC^2 => V(x2 - x1)^2 + (y2 - y1)^2
...
function getLinearEquationRoot(a, b) {
  // a * x = b; x = b / a; | a * x + b = 0; a * x = - b; x = - b / a;
  if (a !== 0) {
    return -b / a;
  }
  return null;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  // |b| = Vx2^2+y2^2
  // ArcCos = a*b / |a|*|b|
  const vectSumm = x1 * x2 + y1 * y2;
  const vectLength =
    Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(vectSumm / vectLength);
}
...
function roundToPowerOfTen(num, pow) {
  ...
  //! 10^pow: 10^0 = 1, 10^1 = 10, 10^2 = 100, 10^3 = 1000
...
function isNumber(number) {
  // return Number.isInteger(number); but a number should be an integer
...
function roundToSmallestInteger(number) {
  //* Actually, The Math.ceil() static method always rounds up and returns the smallest integer greater than or equal to a given number.
...
function getCountOfOddNumbers(number) {
  // Math.abs is an important method here
  let countOdd = 0;
